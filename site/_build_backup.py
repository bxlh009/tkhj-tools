...
def page(title, body_str, active="home", breadcrumb=None):
    meta = [
        '<meta charset="utf-8">',
        ...
        '<style>.breadcrumb{padding:12px 0;font-size:.85rem;color:var(--text-3)}.breadcrumb a{color:var(--text-3);text-decoration:none}.breadcrumb a:hover{color:var(--brand)}.breadcrumb span{margin:0 8px;color:var(--text-3)}</style>',
        AD_HEAD,
    ]
    breadcrumb_html = ""
    if breadcrumb:
        items = ['<a href="/">Home</a>'] + breadcrumb
        breadcrumb_html = '<div class="container breadcrumb">' + '<span>?</span>'.join(items) + "</div>"
    return "<!doctype html><html lang=\"en\" data-theme=\"light\"><head>" + NL.join(meta) + "</head><body><script>try{var t=localStorage.getItem('tkhj-theme');if(t)document.documentElement.setAttribute('data-theme',t);}catch(e){}</script>" + nav(active) + "<main>" + breadcrumb_html + body_str + "</main>" + footer() + '<script src="/' + JSL + '"></script></body></html>'
...
def about_page():
    ...
    body += '<p><strong style="color:var(--text-1)">How this site works:</strong> All articles start as AI drafts, then I personally review, fact-check, and edit every piece before it goes live. The exam strategies come from real classroom experience; the AI tool reviews come from hands-on testing. If a claim cannot be verified, it is removed or rewritten.</p>'
...
def main():
    ...
    four_oh_four = '<section style="padding:80px 0;text-align:center"><div class="container">'
four_oh_four += '<h1 style="font-size:clamp(2rem,5vw,3.5rem);margin:0 0 16px;color:var(--text-1)">404</h1>'
four_oh_four += '<p style="margin:0 0 24px;color:var(--text-2);font-size:1.1rem">Sorry, that page doesn\'t exist.</p>'
four_oh_four += '<a href="/" style="display:inline-block;padding:12px 24px;background:var(--brand);color:#fff;text-decoration:none;border-radius:8px;font-weight:600">Back to Home</a>'
four_oh_four += '</div></section>'
wp("404.html", page("Page Not Found", four_oh_four))

# Generate sitemap.xml
from datetime import datetime as _dt
today = _dt.now().strftime("%Y-%m-%d")
sitemap = '<?xml version="1.0" encoding="UTF-8"?>\n'
sitemap += '<urlset xmlns="http://www.sitemapindex.org/schemas/sitemap/0.9">\n'
for a in exam_arts + ai_arts:
    slug = a["_slug"]
    section = "exam" if a.get("exam") else "ai"
    url = "https://tkhjtools.top/" + section + "/article/" + slug + ".html"
    sitemap += "  <url><loc>" + url + "</loc><lastmod>" + today + "</lastmod></url>\n"
sitemap += "</urlset>"
(OUT / "sitemap.xml").write_text(sitemap, encoding="utf-8")

# Generate robots.txt
robots = "User-agent: *\nAllow: /\n\nSitemap: https://tkhjtools.top/sitemap.xml"
(OUT / "robots.txt").write_text(robots, encoding="utf-8")
    print("  sitemap.xml:", len(exam_arts + ai_arts), "URLs")
